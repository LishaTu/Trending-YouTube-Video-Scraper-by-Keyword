...
class SearchHandler:
    ...
    def search_videos_with_keywords(self, keywords: List[str], 
                                  ...
                                  search_in_title_only: bool = True) -> List[Dict[str, Any]]:
        """
        Search videos with multiple keywords
        
        Args:
            keywords: List of keywords to search
            max_total_results: Maximum total results to fetch across all pages
            order: Sort order
            published_after: RFC 3339 formatted date-time
            published_before: RFC 3339 formatted date-time
            search_in_title_only: If True, search only in titles using intitle: operator
        ...
        if search_in_title_only:
            # Use intitle: operator for each keyword
            # This will find videos that have ALL keywords in the title
            query = ' '.join([f'intitle:"{keyword}"' for keyword in keywords])
            
            # Alternative: To find videos with ANY of the keywords in title:
            # query = ' OR '.join([f'intitle:"{keyword}"' for keyword in keywords])
        else:
        ...
        print(f"Search mode: {'Title only' if search_in_title_only else 'All fields'}")
        if published_after:
    ...
    # Alternative method for more flexible title searching
    def search_videos_title_any_keyword(self, keywords: List[str], 
                                      max_total_results: int = 100,
                                      order: str = 'viewCount',
                                      published_after: Optional[str] = None,
                                      published_before: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        Search videos that have ANY of the keywords in the title
        ...
        Returns:
            List of video items
        """
        # Build query to find videos with ANY keyword in title
        query = ' OR '.join([f'intitle:"{keyword}"' for keyword in keywords])
        
        all_videos = []
        next_page_token = None
        
        print(f"Search query (ANY keyword in title): '{query}'")
    ...
    def search_and_get_details(self, keywords: List[str], 
                             ...
                             published_after: Optional[str] = None,
                             published_before: Optional[str] = None,
                             title_search_mode: str = 'all') -> List[Dict[str, Any]]:
        """
        Combined search and detail fetching with title search options
        ...
        Args:
            ...
            published_after: RFC 3339 formatted date-time
            published_before: RFC 3339 formatted date-time
            title_search_mode: 'all' (all keywords in title), 'any' (any keyword in title), or 'general' (search all fields)
            
        Returns:
            List of videos with full details
        """
        # Search for videos based on mode
        if title_search_mode == 'any':
            search_results = self.search_videos_title_any_keyword(
                keywords, 
                max_results,
                published_after=published_after,
                published_before=published_before
            ...
            # 'all' or 'general' mode
            search_in_title_only = (title_search_mode == 'all')
